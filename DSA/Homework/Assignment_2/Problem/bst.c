...
int max_value(SearchTree T) //maximum value of a general binary tree.
{
    if (T == NULL)
        return -1000000;
...
//     return (totalHeight(T, P->left) + height_2(P) + (totalHeight(T, P->right)));
// }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 0; j < 4; j++) //iterating for each value of m
        {
            ...
            for (int k = 0; k < m; k++) //running 'm' times
            {
                ...
                // total_height += totalHeight(Y, Y);
                total_height += height_2(Y);
            }
            Plot[p % 4][q] = (double)total_height / m;
            p++;
        }
        q++;
    }
